...
public class DividendJob extends Configured implements Tool {
	...
	public static class DividendGrowthMapper extends
			Mapper<LongWritable, Text, Stock, DoubleWritable> {
	...
	public static class StockPartitioner extends
			Partitioner<Stock, DoubleWritable> {

		@Override
		public int getPartition(Stock key, DoubleWritable value,
				int numReduceTasks) {

			char firstLetter = key.getSymbol().trim().charAt(0);
			return firstLetter % numReduceTasks;
		}
	}

	public static class DividendGrowthReducer extends
			Reducer<Stock, DoubleWritable, NullWritable, DividendChange> {
	...
	public static void main(String[] args) {
		int result = 0;
		try {
			result = ToolRunner.run(new Configuration(), new DividendJob(),
					args);
		} catch (Exception e) {
			e.printStackTrace();
		}
